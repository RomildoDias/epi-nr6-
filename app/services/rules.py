...
def get_epi_status(epi: models_db.EPI, dias_alerta: int = 30) -> str:
    """
    BUG FIX: ordem de prioridade correta.
    CA_VENCIDO > ABAIXO_MINIMO > CA_ALERTA > OK
    Antes: CA_ALERTA nunca era detectado se estoque também estivesse baixo.
    """
    today = date.today()
...
def get_dashboard_kpis(db: Session, current_user) -> dict:
    ...
    # BUG FIX: query duplicada removida — antes fazia 2 queries para o mesmo dado
    cutoff   = datetime.utcnow() - timedelta(days=7)
    q_colab  = db.query(models_db.Entrega.colaborador_id).filter(
        models_db.Entrega.data >= cutoff)
    q_colab  = tenant_filter(q_colab, models_db.Entrega, current_user)
...
def get_consumo_por_setor(db: Session, current_user, dias: int = 30) -> list:
    """BUG FIX: query duplicada removida — antes executava 2 queries idênticas."""
    ...
    result = tenant_filter(result, models_db.Entrega, current_user)
...
def registrar_entrega(db: Session, current_user,
    ...
    """
    BUG FIX: valida colaborador existe e pertence ao tenant.
    BUG FIX: verifica quantidade > 0 ANTES de verificar saldo.
    """
    import uuid

    # Valida quantidade primeiro
    if quantidade <= 0:
