...
from app.core.security import (
    ...
    hash_password, verify_password, create_access_token, HIERARQUIA_PERFIL,
)
...
def update_usuario(uid: str, body: schemas.UsuarioUpdate,
    ...
    if current_user.perfil != "superadmin":
        if u.tenant_id != current_user.tenant_id:
            raise HTTPException(403, "Você só pode editar usuários do seu tenant")
    if body.perfil and HIERARQUIA_PERFIL.get(body.perfil, 0) > HIERARQUIA_PERFIL.get(current_user.perfil, 0):
        raise HTTPException(403, "Você não pode atribuir um perfil superior ao seu")
...
def download_ficha(eid: str, db: Session = Depends(get_db),
    ...
    q = db.query(models_db.Entrega).filter(models_db.Entrega.id == eid)
    q = tenant_filter(q, models_db.Entrega, current_user)
    entrega = q.first()
...
def entrada_estoque(epi_id: str, body: schemas.MovCreate,
    ...
    q = db.query(models_db.EPI).filter(models_db.EPI.id == epi_id)
    q = tenant_filter(q, models_db.EPI, current_user)
    epi = q.first()
    if not epi: raise HTTPException(404)
...
def ajuste_estoque(epi_id: str, body: schemas.MovCreate,
    ...
    q = db.query(models_db.EPI).with_for_update().filter(models_db.EPI.id == epi_id)
    q = tenant_filter(q, models_db.EPI, current_user)
    epi = q.first()
    if not epi: raise HTTPException(404)
...
def _gerar_qrcode(epi_id: str):
    try:
        ...
        qr.add_data(f"{settings.APP_NAME}/epi/{epi_id}")
        qr.make(fit=True)
        img = qr.make_image()
